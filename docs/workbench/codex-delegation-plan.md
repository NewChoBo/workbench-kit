...
- **첫 권장 작업:** Track D D3 known cleanup, JDW B-UX5 drag/reparent indicators, and editor layout hardening are closed. 다음은 **JDW validation/source polish**, 또는 shell 쪽이면 host-backed storage / install-state planning을 진행한다.
...
```text
JDW validation/source polish -> host-backed storage / install-state planning
```
