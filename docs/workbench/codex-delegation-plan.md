...
- **첫 권장 작업:** Track D D3 known cleanup, JDW B-UX5 drag/reparent indicators, editor layout hardening, source range polish, semantic source validation problems, outline horizontal navigation, outline root-drop edge handling, outline Storybook edge coverage, and workspace host save gating are closed. 다음은 shell 쪽이면 **host-backed storage / install-state planning**, JDW 쪽이면 zoom/pan policy decision을 진행한다.
...
```text
Host-backed storage / install-state planning -> JDW zoom/pan policy decision
```
