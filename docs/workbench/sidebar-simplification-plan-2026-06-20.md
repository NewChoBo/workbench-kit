...
- Uses `ResizeObserver`, CSS var `--ui-sidebar-footer-height`, `SideBarScrollSpacer`
...
| 2026-06-20 | Overlay footer **keep**                                      | Playwright smoke on `workbench-sample` showed Commands status footer and Chat composer overlay both preserve reachable final content via measured spacer (`--ui-sidebar-footer-height`) |
