...
Shallow JSON form in `shell-react` (`JSON_FORM_PROVIDER` / `JsonObjectFormView`) is
opt-in, not schema-driven, and is **not** in the default editor view providers.
Plain `.json` stays Code-only unless a host registers a form provider.

## Testing with `jdw/showcase/example.jdw.json`

1. `pnpm --filter workbench-sample dev`
2. Open `jdw/showcase/example.jdw.json` in the sample workbench.
