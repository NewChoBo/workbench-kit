...
- 단, `@newchobo-ui/react`는 shell 레이아웃 진입점은 추출/공개했지만,
...
| 런타임 bootstrap | `@newchobo-ui/vscode-extension`의 `createWorkbenchExtensionRuntime` 제공 | extension 전용 bootstrap은 있어도 package-level 앱 엔트리와 분리됨 |
| 저장/채팅/패치 서비스 계층 | `@newchobo-ui/services`, `@newchobo-ui/adapters` 구성 완료 | 서비스 호출 주체를 story가 아닌 host/앱 조립기로 이동 필요 |
...
`@newchobo-ui/react`의 공개 API는 shell을 노출하고 있으나, 앱 런타임을 즉시 임포트 가능한 완전한 조합체로 만들려면
...
`@newchobo-ui/react/workbench`에 실제 앱이 바로 import 가능한 워크벤치 엔트리포인트를 만든다.
...
- `@newchobo-ui/vscode-host` test/typecheck
- `@newchobo-ui/services` typecheck
...
#### 게이트
- `@newchobo-ui/vscode-extension` test/typecheck
...
#### Step 4 상세 산출물
- `@newchobo-ui/vscode-extension`의 사용 가이드 문서 1건
...
export interface WorkbenchShellProps<TActivityId extends string = 'explorer' | 'search' | 'chat', TTheme extends string = 'light' | 'dark'> {
  title?: string;
...
### 변경 후(레인 A 완료 시)
- 앱 소비자가 `@newchobo-ui/react/workbench`에서 진입점 사용 가능
