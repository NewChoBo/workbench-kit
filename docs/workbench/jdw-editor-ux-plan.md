...
- **핵심 UX 갭:** 아웃라인 reorder/reparent/collapse/drop-position, asset-to-outline drop, Monaco reveal/sync, persistent outline + Props/Assets detail tabs, selected canvas frame + stack/grid drag/stack southeast resize commit은 완료. hover/focus chrome, reparent/grid reflow, asset-to-preview drop, 나머지 resize edge, 줌/팬은 남았다.
- **개선 방향:** Figma 클론이 아니라 **JDW 단일 SSoT + 커밋형 제스처**([jdw-schema-figma-authoring.md](./jdw-schema-figma-authoring.md)). 정적 preview selection, editor discipline, outline ergonomics core, stack placement controls/preview geometry, selected frame drag/resize commit은 required Storybook play로 고정했다. 다음은 reparent/grid reflow edge를 B4로 분리해서 진행한다.
...
- **다음 권장:** **B4 reparent/grid reflow edge** — B3 first wire-in과 stack southeast resize는 `WidgetTreeCanvasPreview`로 연결됐으므로 reparent gestures, grid reflow/slot edge, 나머지 resize handles를 좁은 headless+React smoke slice로 나눠 진행한다.
...
| `JDW/WidgetTree/Lab` · Stack placement           | same                        | Stack placement controls, preview geometry, selected frame drag + southeast resize commit |
...
| P7  | **Placement inspector partial**                                                      | Stack inset fields are labelled, editable, patch-covered, draggable, and southeast-resizable on canvas; registry inspector fields remain demo-limited            | Low      |
...
5. **Commit-on-pointer-up** for canvas gestures — stack/grid selected-frame drag and stack southeast resize now commit through JDW patches; reparent/grid reflow remain future.
...
| Item                    | Action                                                                                                         |
| ----------------------- | -------------------------------------------------------------------------------------------------------------- |
| Stack placement section | Done for labelled stack child inset fields, patch updates, preview geometry, canvas drag, and southeast resize |
...
| Gestures                       | Done for selected stack/grid placement drag and stack southeast resize; drag reparent, grid reflow, and other resize handles remain B4 |
| Zoom / pan                     | **Only if** Lane C policy reverses; default remains deferred                                                                           |

**Acceptance criteria**

- [x] Story: canvas selected frame matches tree selection; stack drag and southeast resize update JSON via patch
...
- [ ] Story: drag reparent / grid reflow / remaining resize edge updates JSON via patch
...
| B4 Drag reparent / reflow | UX-5 polish                   | Stack southeast resize landed; reparent gestures, grid reflow, remaining resize handles, optional zoom overlap with Lane C |

**Sequencing rule:** Track B-UX **deferred until WB-29 closeout**; UX-1–UX-4 core, the B3 first canvas wire-in, and stack southeast resize are now landed. Remaining UX-5 work should be treated as B4 edge slices with dedicated headless + Storybook coverage.
