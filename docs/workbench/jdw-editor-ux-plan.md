...
- **다음 권장:** Lane A S12 closeout은 완료. JDW에 남는다면 outline Storybook edge coverage 또는 preview zoom/pan 정책 결정을 좁은 slice로 진행한다.
...
| P5  | **Outline drop-position core covered; edge stories remain** | Same-parent/cross-parent before/after/inside operation resolution, row affordance, root inside-only drop placement, and stale invalid drag-over cleanup are wired; required play covers reorder/reparent, not every drop edge                                                                                                          | Low      |
