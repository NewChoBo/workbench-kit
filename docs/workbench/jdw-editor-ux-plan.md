...
- **다음 권장:** Lane A S12 closeout은 완료. JDW에 남는다면 outline edge polish, validation host parity, 또는 preview zoom/pan 정책 결정을 좁은 slice로 진행한다.
...
| P3  | **Validation source feedback wired; host parity remains**   | `WidgetTreeWorkbench` renders `JsonConfigValidationBanner`, disables invalid save, and has required play coverage; `WidgetTreeLab` maps semantic validation issues to source problems near widget ranges; workspace shell policy remains host-dependent                                                                                | Low      |
...
**Sequencing rule:** Track B-UX **deferred until WB-29 closeout**; UX-1–UX-4 core, B1 root schema placement parity/per-parent schema refs, the B3 first canvas wire-in, stack 8-way resize, grid columns reflow, canvas reparent, grid drag-slot collision reflow, grid resize span reflow, row/column linear resize, wrapper-child resize, asset preview drop + marker, preview hover/focus chrome, drag/reparent ghost + snap indicators, source range highlight, and semantic validation source problems are now landed. Remaining UX polish should target outline edge feedback, validation host parity, or wait for an explicit zoom/pan policy decision.
