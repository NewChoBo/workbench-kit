...
- **다음 권장:** Lane A S12 closeout은 완료. JDW에 남는다면 preview zoom/pan 정책 결정을 좁은 slice로 진행한다. Shell/library 완성도 쪽이면 host-backed storage / install-state planning으로 넘어간다.
...
| B4 Drag reparent / reflow | UX-5 polish                   | Stack 8-way resize, grid columns reflow, canvas reparent, grid drag-slot collision reflow, grid resize span reflow, row/column linear resize, wrapper-child resize, asset preview drop + marker, and drag/reparent ghost + snap indicators landed; optional zoom overlap remains Lane C |

**Sequencing rule:** Track B-UX **deferred until WB-29 closeout**; UX-1–UX-4 core, B1 root schema placement parity/per-parent schema refs, the B3 first canvas wire-in, stack 8-way resize, grid columns reflow, canvas reparent, grid drag-slot collision reflow, grid resize span reflow, row/column linear resize, wrapper-child resize, asset preview drop + marker, preview hover/focus chrome, drag/reparent ghost + snap indicators, source range highlight, semantic validation source problems, outline ArrowLeft/Right navigation, outline keyboard focus follow-up, outline root-drop edge handling, outline Storybook edge coverage, and workspace host save gating are now landed. Remaining JDW UX polish should wait for an explicit zoom/pan policy decision.
