...
- **다음 권장:** Lane A S12 closeout은 완료. JDW에 남는다면 drag/reparent ghost indicators와 snap indicators를 좁은 slice로 진행한다.
...
| --- | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| P1  | **Preview chrome still shallow** — selected, hover, focus, and asset drop marker exist; drag/reparent ghost indicators remain | `WidgetTreeCanvasPreview` overlays selected layout frames with drag/resize handles plus transient hover/focus frames and asset drop parent/index/slot markers; live drag/reparent ghosts remain polish                                                                                                                                 | Low      |
...
| B4 Drag reparent / reflow | UX-5 polish                   | Stack 8-way resize, grid columns reflow, canvas reparent, grid drag-slot collision reflow, grid resize span reflow, row/column linear resize, wrapper-child resize, asset preview drop, and asset preview drop marker landed; optional zoom overlap remains Lane C |

**Sequencing rule:** Track B-UX **deferred until WB-29 closeout**; UX-1–UX-4 core, B1 root schema placement parity/per-parent schema refs, the B3 first canvas wire-in, stack 8-way resize, grid columns reflow, canvas reparent, grid drag-slot collision reflow, grid resize span reflow, row/column linear resize, wrapper-child resize, asset preview drop + marker, and preview hover/focus chrome are now landed. Remaining UX polish should target drag/reparent ghost indicators with dedicated Storybook coverage.
