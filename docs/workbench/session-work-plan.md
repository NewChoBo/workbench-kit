...
| —   | **D-S1** | Track D3 legacy shim audit                                  | **Done** — capability, URI, and editor-host context cleanup  |
...
- **다음 3세션:**
  1. **Host-backed storage / install state** — DOM-free storage port is in place; next is a concrete file-backed adapter and plugin install/update plan.
...
| **Date**            | 2026-06-25                                                                                         |
| **Branch**          | `feature/theia-strengths-workbench`                                                                |
| **Working tree**    | Host-neutral storage port for shell persistence and install-state                                  |
| **Last commits**    | JDW outline edge coverage; outline and host save parity                                            |
| **Lane A progress** | Complete (WB-23–WB-31 plus S12 DoD audit)                                                          |
| **Validate note**   | `pnpm validate:full` green 2026-06-25; Vitest 207 files / 953 tests; Storybook required play 30/30 |
...
| D3    | Done 2026-06-25 | Legacy shim removal                       |
...
Inventory and low-risk cleanup can run in parallel. Builtin render unification and known legacy shim cleanup are complete; D4 doc truth stays continuous.
...
| D3    | Drop legacy compat shims (URI models, editor scaffold trim; capability seed done)             | Done     | Done 2026-06-25         | No                |
...
- Generic `ResourceUri` vs `WorkspaceResourceUri` dual model remains in
  ...
  dependency.
- Done 2026-06-25: `EditorHostCreateContext.resourceUri` is required and the
  unused `tabId` creation context was removed after `builtin.editor` and
  `builtin.commands` proved editor hosts only need resource identity.
...
| **WB-29 selection/reveal closeout**  | Explorer/editor sync needs periodic smoke coverage      | Recheck command handlers and sample flows before Lane A DoD                      |
...
- [x] B-UX2 — Outline DnD reorder + keyboard nav (UX-2)
...
- [x] B-UX5 edge — Drag/reparent ghost and snap indicators
...
- [x] D3 — Drop legacy compat shims (post-Lane A)
...
| -------- | ---------------------------------------------------------------------------- | ------------- | ----------------------------------------------- | --------------------------------------------- |
| P0       | **Host-backed storage / install state plan**                                 | Parallel-safe | `shell-react/provider`, install-state storage   | storage port landed; file-backed adapter next |
...
**Suggested next slice:** Concrete host file-backed storage adapter / plugin install-state update plan, or preview zoom/pan policy if staying in JDW.
...
| Date       | Note                                                                                                                                                                                                                                 |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-06-25 | Validation: `pnpm validate:full` green for host-neutral storage/install-state; Vitest 207 files / 953 tests and Storybook required play 30/30                                                                                        |
| 2026-06-25 | Extension install planning now filters unrelated enabled-extension diagnostics out of catalog install plans while still checking diagnostics that affect the planned target/dependencies/providers                                   |
| 2026-06-25 | Sample host install-state storage: the sample app injects an account-scoped installed-extension storage adapter, and Storybook exposes a host-provided preinstalled catalog extension through the provider snapshot                  |
| 2026-06-25 | Host-neutral storage port: `workbench-core` exports DOM-free storage reader/writer/adapter types, install-state helpers consume them, and `shell-react` persistence helpers no longer expose DOM `Storage` in their adapter contract |
...
| 2026-06-25 | JDW outline horizontal navigation: ArrowLeft/Right now resolve tree collapse, expand, parent selection, and first-child selection actions                                                                                            |
| 2026-06-25 | Validation: `pnpm validate:full` green for JDW semantic validation source problems; Vitest 204 files / 938 tests and Storybook required play 29/29                                                                                   |
| 2026-06-25 | JDW semantic validation source problems: validation issues now appear as Monaco problem entries located at the nearest widget source range instead of being treated as parse errors                                                  |
| 2026-06-25 | Validation: `pnpm validate:full` green for JDW source selection range polish; Vitest 204 files / 935 tests and Storybook required play 29/29                                                                                         |
| 2026-06-25 | JDW source selection range: selected widget paths now resolve full source object ranges, including JDW v7 `args` wrappers, and the shared JSON editor decorates the active Monaco range                                              |
| 2026-06-25 | Validation: `pnpm validate:full` green for editor layout ownership hardening; Vitest 204 files / 927 tests and Storybook required play 29/29                                                                                         |
...
| 2026-06-25 | S12 Lane A closeout audit documented the final DoD evidence; Lane A is complete and post-Lane A cleanup moved to Track D and JDW polish                                                                                              |
