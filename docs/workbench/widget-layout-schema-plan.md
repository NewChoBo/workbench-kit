...
| Patch / tree ops                     | `applyWidgetPatch` + `normalizeWidgetForParent`         | Outline reorder/reparent and asset drop are wired; headless stack/grid drag + stack 8-way resize mapping exists; selected React preview/canvas stack/grid drag, stack resize, and grid columns reflow are wired; reparent/grid drag gestures deferred |
...
| D4  | Grid columns change reflow                      | Implemented for direct child order reflow through `reflowGridChildren`; broader occupancy/drag-slot policy remains B4 edge                      |
