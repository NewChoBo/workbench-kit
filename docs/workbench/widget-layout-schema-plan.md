...
- [x] Stack southeast resize mapping: canvas resize handle commits `right`/`bottom` placement updates through `WidgetPatch`
...
| Patch / tree ops                     | `applyWidgetPatch` + `normalizeWidgetForParent`         | Outline reorder/reparent and asset drop are wired; headless stack/grid drag + stack resize mapping exists; selected React preview/canvas stack/grid drag and southeast resize are wired; reparent/grid reflow gestures deferred |
