...
| Patch / tree ops                     | `applyWidgetPatch` + `normalizeWidgetForParent`         | Outline reorder/reparent and asset drop are wired; headless stack/grid drag + stack 8-way resize mapping exists; selected React preview/canvas stack/grid drag and stack resize are wired; reparent/grid reflow gestures deferred |
