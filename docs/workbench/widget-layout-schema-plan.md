...
- [ ] Grid drag-slot and non-stack resize edge policy
...
| Patch / tree ops                     | `applyWidgetPatch` + `normalizeWidgetForParent`         | Outline reorder/reparent and asset drop are wired; headless stack/grid drag + stack 8-way resize + canvas reparent mapping exists; selected React preview/canvas stack/grid drag, stack resize, grid columns reflow, and canvas reparent are wired; grid drag-slot gestures deferred |
