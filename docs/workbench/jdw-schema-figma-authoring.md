...
1. **Canvas authoring pipeline is still narrow** — `WidgetTreeCanvasPreview` wraps `JdwPreview` with selected layout frames and now accepts asset preview drops, but richer before/after placement indicators and hover/focus chrome remain.
