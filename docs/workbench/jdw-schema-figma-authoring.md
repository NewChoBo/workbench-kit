...
| Canvas drag / resize → JDW              | **Missing (React UI)** | Headless drag patch base exists; no canvas event wiring or resize mapper   |
...
2. **No React canvas gesture pipeline** — headless hit-test and stack/grid drag mapping now exist, but pointer events are not wired from preview/canvas frames into JDW patches.
3. **Functional resize handles** exist as layout primitives but do not emit JDW placement updates.
4. **Phase 4 checklist incomplete** — outline DnD, asset materialization, and the B2 headless mapping base are wired; canvas gestures, resize, reparent polish, and layout-driven preview promotion remain.
