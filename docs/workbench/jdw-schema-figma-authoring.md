...
| Headless mapping layer                  | **Adopted (base)**     | `layout-mapping.ts` hit-test + stack/grid drag/reparent patch tests                                          |
...
| Canvas drag / resize → JDW              | **Partial**            | Selected stack/grid drag, stack 8-way resize, and canvas reparent commit JDW patches; grid drag edge remains |
| Inspector placement reflow              | **Partial**            | Grid `columns` edits reflow direct child placement through JDW patches; broader grid drag/slot edge remains  |
...
2. **React canvas gesture pipeline is partial** — selected stack/grid drag, stack 8-way resize, and canvas reparent commit JDW patches; grid drag/slot edge coverage remains.
...
4. **Phase 4 checklist incomplete** — outline DnD, asset materialization, the B2 headless mapping base, the B3 first canvas wire-in, stack resize, grid column reflow, and canvas reparent are wired; broader layout-driven edge promotion remains.
...
| **B2 (base done)** | **Mapping layer spec** — hit-test on layout rects; stack/grid drag/reparent → `WidgetPatch` | `layout-mapping` headless tests                                                             |
...
| **B4**             | Drag reparent, resize, grid reflow, optional zoom/pan (Lane C overlap)                      | Stack 8-way resize, grid columns reflow, and canvas reparent landed; grid drag edge remains |
