...
| WB-20 | done     | P1       | Editor     | Resource draft controller                     | WB-07, WB-12, WB-15                 | `@workbench-kit/workspace`, `@workbench-kit/react`                                                      | Generic draft/save/discard helpers, dirty draft path queries, and a shared React draft controller cover file and structured preview editors.                                          |
...
| WB-23 | done     | P0       | Harness    | Workbench sample host + launch boundary scope | Current develop baseline            | `examples/workbench-sample`, `scripts`                                                                  | Minimal Vite host at `pnpm workbench-sample`; reads `.workbench` extensions/layout.                                                                                                   |
| WB-24 | done     | P1       | Lifecycle  | ViewHost lifecycle contract                   | Existing ViewHost                   | `@workbench-kit/workbench-extension-sdk`, `@workbench-kit/shell-react`                                  | Adds optional title/icon/closable metadata, show/hide/focus/blur/resize hooks, and codicon-backed activity icons without breaking existing providers.                                 |
| WB-25 | done     | P1       | Registry   | View/editor host factory registry             | WB-24                               | `@workbench-kit/workbench-core`, `@workbench-kit/workbench-extension-sdk`                               | `ViewHostFactoryRegistry` + default provider factory; `shell-react` resolves hosts through factories.                                                                                 |
...
| WB-28 | done     | P1       | Editor     | Editor contribution and service model         | WB-24, WB-25, WB-27                 | `@workbench-kit/workbench-core`, `@workbench-kit/shell-react`, `@workbench-kit/workbench-extension-sdk` | Tab chrome, `EditorArea`, builtin text editor, Code(JSON)/Form/Preview sample editing, and save via `WorkspaceResourceTransaction` are landed; explorer open/reveal remains in WB-29. |
| WB-29 | done     | P2       | Workspace  | Command-backed built-in explorer              | WB-24, WB-26, WB-27                 | `extensions/builtin.explorer`, `@workbench-kit/workspace`, `@workbench-kit/shell-react`                 | Command handlers, UI dispatch, selection/reveal/search sync, targeted integration tests, and sample browser smoke are landed.                                                         |
| WB-30 | done     | P2       | Config     | Preference scope and merge order              | Existing configuration contribution | `@workbench-kit/workbench-config`, `@workbench-kit/platform`                                            | Start with default/workspace/local preference scopes and leave user/resource/secret scopes explicit for later hardening.                                                              |
| WB-31 | done     | P3       | Devtools   | Registry and lifecycle inspectors             | WB-24, WB-26, WB-27                 | `@workbench-kit/shell-react`                                                                            | Storybook devtools panel with registry snapshots and lifecycle inspectors.                                                                                                            |
...
| 6     | WB-28       | Editor contribution and service model                         | **Done; explorer open/reveal deferred to WB-29** |
| 7     | WB-29       | Command-backed built-in explorer                              | **Done** (2026-06-20 closeout)                   |
...
type WorkbenchConfirmationSideEffect =
  'none' | 'workspace-write' | 'external-write' | (string & {});
