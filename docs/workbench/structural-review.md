...
- **이중 문서 모델:** 위젯 영속화는 JDW v7 단일 SSoT. `WorkbenchDocument`(절대 좌표 캔버스)는 `WorkbenchCanvasShell` 데모 전용이며 위젯 파일과 혼용 금지.
- **Lane A 갭 (WB-28):** `EditorService`·`useEditor*`는 구현됐으나 `WorkbenchShell`은 여전히 `editorArea` placeholder를 호출자가 주입해야 함. 샘플 호스트는 정적 카드만 표시.
