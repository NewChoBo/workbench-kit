...
- [`component-rendering-contract.md`](./component-rendering-contract.md) — source-backed descriptor/renderer gaps, shared rendering semantics, Preview/runtime conformance, Web Components delivery relationship, and the design-only rendering prerequisite
...
- **CURRENT INTEGRATION BASELINE** — behavior or structure verified against the explicitly named integration baseline
- **HISTORICAL SOURCE SNAPSHOT EVIDENCE** — reviewed historical source that can inform a target but is not a current integration fact until re-verified
- **CURRENT SOURCE FACT** — verified behavior or structure in source/tests/packages for the explicitly named current integration baseline
