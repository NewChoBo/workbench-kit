...
M0-M5 are complete for the in-repo migration baseline. Bulk replacement is acceptable for in-repo consumers, and prototype consumers should move directly to the target package surfaces. The legacy `@workbench-kit/core` and VS Code bridge packages have been removed; see [Migration Strategy](./migration-strategy.md).
