...
- **UI-agnostic**: View contributions describe _what_ to show; `shell-react` decides _how_ to render.
...
contributions. `shell-react` calls `createViewHost` instead of invoking
...
- `activateView(viewId)` activates matching `onView:` extensions before `shell-react` mounts a registered view provider.
