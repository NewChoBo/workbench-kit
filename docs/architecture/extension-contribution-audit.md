...
| Capabilities, dependencies, and permissions                                                   | `capabilities`, extension dependency fields, `permissions`                                            | Supported | Hard extension dependencies are validated after registration. Required-capability availability is reported through dependency diagnostics, while sensitive capability declarations and permissions are enforced when an activated extension requests the capability. None of these declarations grants host authority by itself. |
