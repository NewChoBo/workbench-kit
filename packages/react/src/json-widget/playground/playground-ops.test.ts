...
import { WELCOME_PLAYGROUND_DOCUMENT } from './demo-registry.js';
import {
...
describe('playground widget ops', () => {
  ...
  it('deletes multiple selected widgets in one operation', () => {
    const document = duplicatePlaygroundWidget(WELCOME_PLAYGROUND_DOCUMENT, [
      { kind: 'children', index: 0 },
    ]);
    expect(document).not.toBeNull();

    const next = deletePlaygroundWidgets(document!, [
      [{ kind: 'children', index: 0 }],
      [{ kind: 'children', index: 1 }],
    ]);
    expect(next).not.toBeNull();
    expect(getWidgetChildren(parseWidgetJson<GenericWidget>(next!).value!)).toHaveLength(0);
  });
