...
const STORY_PARTS_DOCUMENT_ASSET_CATALOG = mergeWidgetAssetCatalogs(
  WIDGET_TREE_DEMO_ASSET_CATALOG,
  createWidgetAssetCatalog([
    {
      id: 'jdw-doc:jdw/parts/header/header.jdw.json',
      label: 'header',
      description: 'Workspace JDW · jdw/parts/header/header.jdw.json',
      category: 'parts',
      kind: 'container',
      placementPolicy: 'preserve-internal-layout',
      icon: 'codicon-file',
      packagePath: 'jdw/parts/header',
      content: {
        type: 'column',
        gap: 4,
        children: [{ type: 'text', text: 'Part Header' }],
      } as GenericWidget,
    },
  ]),
);
...
export const DesignSidebarsLayout: Story = {
  name: 'Design sidebars layout',
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await waitForWidgetTreeSourcePane(canvasElement);

    await expect(canvas.getByTestId('widget-tree-lab-assets-sidebar')).toBeVisible();
    await expect(canvas.getByTestId('widget-tree-lab-main-pane')).toBeVisible();
    await expect(canvas.getByTestId('widget-tree-lab-render-pane')).toBeVisible();
    await expect(canvas.getByTestId('widget-tree-lab-authoring-sidebar')).toBeVisible();
    await expect(canvas.getByTestId('widget-tree-side-panel-outline')).toBeVisible();
    await expect(canvas.getByTestId('widget-tree-side-panel-properties')).toBeVisible();
    await expect(canvas.getByTestId('widget-tree-asset-palette')).toBeVisible();
    await expect(canvas.getByTestId('widget-tree-outline-panel')).toBeVisible();
    expect(canvas.queryByRole('button', { name: 'Assets' })).toBeNull();
    expect(canvas.queryByRole('button', { name: 'Props' })).toBeNull();
  },
  tags: ['storybook-play-required'],
};
...
export const PartsDocumentAssetInsert: Story = {
  name: 'Parts document asset insert',
  render: () => <WidgetTreeStoryHarness assetCatalog={STORY_PARTS_DOCUMENT_ASSET_CATALOG} />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await waitForWidgetTreeSourcePane(canvasElement);
    ...
    await userEvent.click(
      await canvas.findByTestId('widget-asset-jdw-doc:jdw/parts/header/header.jdw.json'),
    );
    ...
    await waitFor(() => {
      ...
      expect(
        canvasElement.querySelector(
          '[data-widget-path="$.children[3]"][data-widget-type="column"]',
        ),
      ).toBeTruthy();
      expect(
        canvasElement.querySelector('[data-widget-path="$.children[3].children[0]"]')
          ?.textContent ?? '',
      ).toContain('Part Header');
    });
  },
  tags: ['storybook-play-required'],
};
...
function WidgetTreeStoryHarness({
  ...
  assetCatalog?: WidgetAssetCatalogContract;
  initialValue?: string;
}) {
