...
describe('WidgetTreeCanvasPreview', () => {
  ...
  it('emits fixed-size resize patches for selected wrapper children', async () => {
    ...
    expect(frame?.getAttribute('style')).toContain('--ui-workbench-canvas-item-width: 120px');
    expect(frame?.getAttribute('style')).toContain('--ui-workbench-canvas-item-height: 70px');
...
describe('WidgetTreeCanvasPreview viewport session', () => {
  afterEach(() => {
    document.body.innerHTML = '';
  });

  it('exposes editor-session zoom/pan controls without mutating the document', async () => {
    const rootWidget: GenericWidget = {
      type: 'text',
      text: 'Hello',
      width: 120,
    ...
    const canvas = container.querySelector('[data-testid="widget-tree-canvas-preview"]');
    expect(canvas).not.toBeNull();
    expect(container.textContent).toContain('Ctrl + Scroll to zoom');
    expect(container.textContent).toContain('Reset (');

    const stage = container.querySelector(
      '.ui-workbench-preview-canvas__stage',
    ) as HTMLElement | null;
    ...
    const reset = container.querySelector(
      '.ui-workbench-preview-canvas__reset',
