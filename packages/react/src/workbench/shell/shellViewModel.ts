...
> {
  ...
  readonly resolveTitle?:
    ((item: WorkbenchViewEditorTabItem<TViewId, TIcon>) => string | undefined) | undefined;
}
...
>): WorkbenchShellProps['activityBar'] {
  ...
  const items = model.items.map(toActivityBarItem);
...
> {
  return [...model.items, ...model.footerItems].map((item) => ({
    icon: item.icon,
    id: item.id,
    label: item.label,
  }));
