...
export interface StoryWorkbenchShellFrameProps {
  ...
  /** Stretch editor/sidebar frames to the Storybook canvas (use with `fullHeightShell`). */
...
export const StoryWorkbenchShellFrame = forwardRef<HTMLDivElement, StoryWorkbenchShellFrameProps>(
  function StoryWorkbenchShellFrame(
    { children, className, fill = false, variant = 'settings' },
    ref,
  ) {
    const classes = [
