...
describe('themePresets', () => {
  ...
  it('applies default shell preset without leaving a stale attribute', () => {
    const root = document.createElement('html');
    root.dataset.shellPreset = 'airy';
    ...
    expect(root.dataset.shellPreset).toBe('default');
    expect(root.dataset.theme).toBe('light');
    expect(root.dataset.themePreset).toBe('skyblue');
  });
});
