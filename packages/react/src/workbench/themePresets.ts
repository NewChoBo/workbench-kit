...
export function applyWorkbenchThemeProviderAttributes(
  root: HTMLElement,
  attributes: WorkbenchThemeProviderAttributes,
): void {
  ...
  if (attributes.themePreset === undefined || attributes.themePreset === '') {
    delete root.dataset.themePreset;
  } else {
    root.dataset.themePreset = attributes.themePreset;
  }

  if (attributes.shellPreset === undefined) {
    delete root.dataset.shellPreset;
  } else {
    applyWorkbenchShellAttributes(root, attributes.shellPreset);
  }
}
...
export interface WorkbenchAppearanceSettings {
  /** VS Code: workbench.preferredDarkColorTheme (color preset id for dark mode). */
  darkPreset: string;
  /** VS Code: workbench.preferredLightColorTheme (color preset id for light mode). */
  lightPreset: string;
  /** Workbench layout metrics preset (`data-shell-preset`); orthogonal to Color Theme. */
  shellPreset: string;
  /** VS Code: window.autoDetectColorScheme + forced light/dark. */
...
export function applyWorkbenchAppearance(
  ...
  applyWorkbenchShellAttributes(root, settings.shellPreset);
