...
import {
  SidebarSectionNestingProvider,
  useSidebarSectionBaseDepth,
} from './SidebarSectionNestingContext';
...
> {
  ...
  headingLevel?: WorkbenchSidebarHeadingLevel;
  /**
   * When true (default), `SideBarListItem` depth under this section is offset by +1
   * so rows read as children of the section header. Set false for trees that already
   * own full indent (rare).
   */
  nestListItems?: boolean;
...
export function WorkbenchSidebarSection({
  ...
  const parentBaseDepth = useSidebarSectionBaseDepth();
  const [uncontrolledCollapsed, setUncontrolledCollapsed] = useState(defaultCollapsed);
  const resolvedCollapsed = collapsed ?? uncontrolledCollapsed;
  const nestedBaseDepth = nestListItems ? parentBaseDepth + 1 : parentBaseDepth;
  ...
  return (
    ...
    >
      ...
      >
        <SidebarSectionNestingProvider baseDepth={nestedBaseDepth}>
          {children}
        </SidebarSectionNestingProvider>
      </div>
    </section>
