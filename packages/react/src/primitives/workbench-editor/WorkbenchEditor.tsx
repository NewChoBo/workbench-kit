...
export interface SegmentedControlOption<TValue extends string = string> {
  /** Accessible name when `label` is icon-only or otherwise non-textual. */
  ariaLabel?: string | undefined;
...
export interface SegmentedControlProps<TValue extends string = string> {
  ...
  /** Match `Button` / toolbar compact density. */
...
export function SegmentedControl<TValue extends string = string>({
  ...
  return (
    <ButtonGroup
      ariaLabel={ariaLabel}
      className={cx('ui-segmented-control', compact && 'ui-segmented-control--compact', className)}
    >
      {options.map((option) => {
        ...
        return (
          <button
            ...
            onClick={() => {
              if (optionDisabled || selected) return;
...
export interface EditorTab {
  ...
  /** Remote image URL for tab icon (e.g. game library artwork). Takes precedence over `icon`. */
...
function EditorTabIcon({
  ...
  if (iconImageUrl) {
    return (
      <WorkbenchMediaSlot
        aria-hidden
        className="ui-editor-tabs__icon-slot"
        fallback={
          <FileIcon
        ...
        imageClassName="ui-editor-tabs__image-icon"
  ...
  return <FileIcon className="ui-editor-tabs__file-icon" icon={icon} kind={fileIconKind} />;
}
