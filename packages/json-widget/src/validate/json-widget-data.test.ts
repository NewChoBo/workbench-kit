...
import { validateJsonWidgetData } from './json-widget-data.js';
...
describe('validateJsonWidgetData', () => {
  ...
  it('reports missing grid child col/row at rest', () => {
    const result = validateJsonWidgetData(
      JSON.stringify({
        type: 'grid',
        args: {
          columns: 2,
          children: [{ type: 'text', args: { text: 'A' } }],
        },
      }),
      { strictKnownTypes: true },
    );
    ...
    expect(result.issues).toEqual(
      expect.arrayContaining([
        {
          path: 'root.args.children[0].args.col',
          message: 'col is required for grid children at rest.',
        },
        {
          path: 'root.args.children[0].args.row',
