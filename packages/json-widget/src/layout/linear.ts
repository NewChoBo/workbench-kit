...
export function computeLinearChildRects(
  ...
  const anyExplicitFlex = children.some((child) => child.flex !== undefined);
  const flexValues = children.map((child) => {
    if (child.flex !== undefined) {
      return child.flex;
    }
    if (child.mainSize !== undefined) {
      return 0;
    }
    // Legacy equal-split only when no sibling opted into explicit flex.
    // Otherwise content-sized siblings keep intrinsic mainSize (or 0).
    return anyExplicitFlex ? 0 : 1;
  });
