  ...
  try {
    ...
    verifyLegacyTypeScriptSubpaths(
      ...
      [
        ...
        "import { resolveWindowOpenLayout, selectWindowDisplayForBounds } from '@workbench-kit/platform/window-geometry';",
        "import { applyWindowFocusablePolicy, applyWindowResidencyPolicy } from '@workbench-kit/platform/window-residency';",
      ],
      [
        'atomicWriteBytes',
        'atomicWriteText',
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'const { resolveWindowOpenLayout, selectWindowDisplayForBounds } =',
        "  require('@workbench-kit/platform/window-geometry');",
        'const { applyWindowFocusablePolicy, applyWindowResidencyPolicy } =',
        ...
        'assert.equal(typeof applyWindowResidencyPolicy, "function");',
        'assert.equal(typeof applyWindowFocusablePolicy, "function");',
        'assert.equal(typeof selectWindowDisplayForBounds, "function");',
        'assert.deepEqual(',
        '  resolveWindowOpenLayout({',
        '    displays: [{ isPrimary: true, workArea: { x: 0, y: 0, width: 1600, height: 900 } }],',
        '    remember: false,',
        '    saved: null,',
        '  }),',
        '  { bounds: { x: 200, y: 50, width: 1200, height: 800 }, isMaximized: false },',
        ');',
  ...
  try {
    ...
    verifyLegacyTypeScriptSubpaths(
      ...
      [
        ...
        "import { resolveWallpaperCropRect } from '@workbench-kit/electron-shell/wallpaper';",
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "const { createEncryptedSecretVault } = require('@workbench-kit/electron-shell/secret-vault');",
        "const { resolveWallpaperCropRect } = require('@workbench-kit/electron-shell/wallpaper');",
        ...
        'assert.equal(typeof createEncryptedSecretVault, "function");',
        'assert.equal(typeof resolveWallpaperCropRect, "function");',
        ...
        '  ),',
        '  { x: 500, y: 0, width: 1500, height: 2000 },',
        ');',
        ...
        '  cipher: {',
        '    decryptString: (bytes) => textDecoder.decode(bytes).slice(4),',
        '    encryptString: (plaintext) => textEncoder.encode(`enc:${plaintext}`),',
        '    isEncryptionAvailable: () => true,',
        '  },',
        '  documentCodec: {',
        '    parse: (plaintext) => JSON.parse(plaintext).secrets,',
        '    serialize: (secrets) => JSON.stringify({ kind: "consumer.vault", secrets }),',
        '  },',
        '  readVault: async () => storedVault,',
        '  writeVault: async (bytes, metadata) => { storedVault = bytes; commitIds.push([...metadata.secretIds]); },',
        '});',
        '(async () => {',
        '  await vault.setSecrets(new Map([["zeta", "z"], ["alpha", "a"]]));',
        '  assert.deepEqual(commitIds, [["alpha", "zeta"]]);',
        '  assert.deepEqual(await vault.getSecrets(["alpha", "missing"]), new Map([["alpha", "a"]]));',
        '})().catch((error) => { console.error(error); process.exit(1); });',
        '',
      ].join('\n'),
    );
